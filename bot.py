    ...
    if "<product_carousel>" in content:
        ...
        try:
            ...
            if match:
                ...
                # Filter out items with known bad/placeholder URLs
                cleaned_data = [
                ...
                # Inject a text placeholder for items with no valid image_url
                # so the product still shows in the carousel instead of disappearing
                for item in cleaned_data:
                    img = item.get("image_url", "")
                    if not img or "http" not in img:
                        name = item.get("name", "Product").replace(" ", "+")[:40]
                        item["image_url"] = f"https://placehold.co/300x200?text={name}"
                if cleaned_data:
